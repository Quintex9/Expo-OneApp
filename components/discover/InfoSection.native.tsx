...
let hasInitializedMapOnce = false;
...
export const InfoSection = memo(function InfoSection({
  ...
  const [hasMountedMap, setHasMountedMap] = useState(hasInitializedMapOnce);

  useEffect(() => {
    if (hasInitializedMapOnce) {
      setHasMountedMap(true);
      return;
    }

    const handle = InteractionManager.runAfterInteractions(() => {
      hasInitializedMapOnce = true;
      setHasMountedMap(true);
    });
  ...
  const mapCoords = useMemo<[number, number]>(() => coordinates ?? [18.0936, 48.3061], [coordinates]);
  const initialRegion = useMemo(() => zoomToRegion(mapCoords, STATIC_MAP_ZOOM), [mapCoords]);
